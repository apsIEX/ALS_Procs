...
Function SendToProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	if(cMode==1)	//energy
		...
		theta=asin(x/ktot)+0.00*ktot
		beta=asin(y/ktot/cos(theta))-0.003*ktot
	...
	if(cMode==3)	//energy
		//roop for extracting each E_binding
		numE=Dimsize (wv,3) 
		deltaE=DimDelta (wv,3) 
		offE=DimOffset (wv,3) 
		make/o/n=(nx,ny,nz, numE) $(wvn+"_4e")
		wave wvout4=$(wvn+"_4e")			
...
Function MapItProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	duplicate/o bmap $(df+"kx"), $(df+"ky"), $(df+"kz"), $(df+"kk"),$(df+"theta"),$(df+"beta")
	wave kx=$(df+"kx"), ky=$(df+"ky"), kz=$(df+"kz"), kk=$(df+"kk"),theta=$(df+"theta"),beta=$(df+"beta")
	if(popnum==1)
	...
	theta=asin(kx/kk)
	beta=asin(ky/kk/cos(theta))
	//bmap=wv(theta*180/pi)(beta*180/pi)(kk)(y) //fast method, inferior quality
	make/o/n=(numAD, numA, numK) $(df+"sub3d")	//theta, beta, ktot
	wave sub3d=$(df+"sub3d")
	setscale/p x dimoffset(wv,0), dimdelta(wv,0), waveunits(wv,0),sub3d
	setscale/p y dimoffset(wv,1), dimdelta(wv,1), waveunits(wv,1),sub3d
	...
	for (i=0;i<numE;i+=1)
		...
		//bmap=sub3d[p][q][0]
		//doupdate
		bmap[][i]=interp3d(sub3d,theta*180/pi,beta*180/pi,kk)
	endfor
	setbmapcolors(df)
End
...
Window ppplot() : Graph
	...
	SetVariable fixedMot1,pos={93,78},size={75,15},title="phi",value= K0
	SetVariable degPerPixel,pos={183,24},size={110,15},proc=SetdegPerPixel,title="degPerPixel"
	...
	SetVariable theta,pos={93,61},size={75,15},title="theta",value= K0
