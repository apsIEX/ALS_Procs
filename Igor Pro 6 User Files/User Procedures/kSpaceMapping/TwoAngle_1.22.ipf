...
//version 1.20 ER fixed interp2d bug (again)
//version 1.21 ER stripped extra return chars from wavenote
//version 1.22 ER fixed missing comma for igor 6.3
/////////
...
proc kplot(w,map,reDim,AngleRangeC,AngleRangeF)
	...
	string wavenote=replacestring(num2char(13),note($w),"")	//wavenote after stripping end of line char
	...
	SetVariable beta_ value= $(df+"vdbeta")
	...
	SetDrawEnv linefgc= (65535,65535,0)
	if($df+"is3d")
		cstyle(0)
...
function/s newkPlot(wn,mapp,doredm,motorwave1,motorwave2)
	...
	variable/g $(df+"data_min"), $(df+"data_max")
	...
	string/g $(df+"cursorType")="plain"	//"plain" or "kgrid"
	variable/g $(df+"CursCircN")=4
	variable/g $(df+"CursCircIncr")=.025
	variable/g $(df+"CursPhiIncr")=120
	variable/g $(df+"CursPhiOfst")=0
	variable/g $(df+"CursPhiRad")=.1
	variable/g $(df+"CursCircsArePoly")=1
	updatecursor(df)
...
function calcAngleData(df)
	...
	nvar datamin=$(df+"data_min")
	nvar datamax=$(df+"data_max")
	nvar phi0=$(df+"phi0")
	wave ad=$(dF+"angdata")
	if(phi0!=0)
		wavestats/q ad0
		datamin=v_min; datamax=v_max
		angle2k(df)	//converts to k and also does phi rotation
		k2angle(df)
		//strip unusually large numbers which sometimes occur at boundaries
		wavestats/q ad//; print v_min, v_max
		ad=selectnumber(ad<datamin,ad,datamin)
		ad=selectnumber(ad>datamax,ad,datamax)
...
function updatecursor(df)
	string df
	svar ct=$(df+"cursorType")
	nvar ccn=$(dF+"cursCircN")
	nvar cci=$(df+"cursCircIncr")
	nvar cpi=$(df+"cursPhiIncr")
	nvar cpo=$(df+"cursPhiOfst")
	nvar cpr=$(df+"cursPhiRad")
	nvar ccap=$(df+"cursCircsArePoly")
	nvar kmap=$(df+"kmap")
	variable/d nphi=360/cpi
	variable isKgrid=(cmpstr(ct,"kgrid")==0)
	
	variable npcirc=1+selectnumber(nphi>0,100,round(100/nphi)*nphi)
	variable np=7 + kmap*iskgrid*(1 + ccn*npcirc + nphi*3)
	make/o/d/n=(np) $(df+"HairY0"),$(df+"HairX0")
	wave hx0=$(df+"HairX0"), hy0=$(df+"HairY0")
	hx0[0]={-Inf,0,Inf,NaN,0,0,0}
	hy0[0]={0,0,0,NaN,Inf,0,-Inf}
	variable/d i,j,pp=8,x0,x1,y0,y1,ppincr
	if(isKgrid*kmap)
		hx0[7]=nan
		hy0[7]=nan
		for(i=0;i<ccn;i+=1)
			if(ccap)
				//polygons
				ppincr=round(100/nphi)
				for(j=0;j<nphi;j+=1)
					x0=cci*(i+1)*cos((j*cpi+cpo)*pi/180)
					x1=cci*(i+1)*cos(((j+1)*cpi+cpo)*pi/180)
					y0=cci*(i+1)*sin((j*cpi+cpo)*pi/180)
					y1=cci*(i+1)*sin(((j+1)*cpi+cpo)*pi/180)
					hx0[pp,pp+ppincr-1]=x0 + (x1-x0)*(p-pp)/(ppincr-(j==(nphi-1)))
					hy0[pp,pp+ppincr-1]=y0 + (y1-y0)*(p-pp)/(ppincr-(j==(nphi-1)))
					pp+=ppincr
				endfor
				hx0[pp]=nan
				hy0[pp]=nan
				...
				//circles
				hx0[pp,pp+99]=cci*(i+1)*cos((p-pp)*2*pi/99); hx0[pp+100]=nan
				hy0[pp,pp+99]=cci*(i+1)*sin((p-pp)*2*pi/99); hy0[pp+100]=nan
				pp+=101
			endif
		...
		for (i=0;i<nphi;i+=1)
			hx0[pp]={-cpr*cos((i*cpi+cpo)*pi/180),cpr*cos((i*cpi+cpo)*pi/180),nan}
			hy0[pp]={-cpr*sin((i*cpi+cpo)*pi/180),cpr*sin((i*cpi+cpo)*pi/180),nan}
			pp+=3
		endfor
...
Function CursorMenuProc(ctrlName,popNum,popStr) : PopupMenuControl
	String ctrlName
	Variable popNum
	String popStr
	string df=getdf()
	svar ct=$(dF+"cursorType")
	ct=selectstring(popnum<=2,ct,popstr[2,inf])
	if(cmpstr(popStr,"set options...")==0)
		nvar ccap=$(df+"cursCircsArePoly")	;variable ccap2=ccap+1
		nvar ccn=$(dF+"cursCircN")			;variable ccn2=ccn
		nvar cci=$(df+"cursCircIncr")			;variable cci2=cci
		nvar cpi=$(df+"cursPhiIncr")			;variable cpi2=cpi
		nvar cpo=$(df+"cursPhiOfst")			;variable cpo2=cpo
		nvar cpr=$(df+"cursPhiRad")			;variable cpr2=cpr
		prompt ccap2,"CursorType",popup "Circles;Polygons"
		prompt ccn2,"Number of Circles"
		prompt cci2,"Increment of Radius"
		prompt cpi2, "Increment of Phi"
		prompt cpo2,"Phi Offset"
		prompt cpr2,"Phi Max Radius"
		doprompt "Enter Cursor Parameters",ccap2,ccn2,cci2,cpi2,cpo2,cpr2
		ccap=ccap2-1
		ccn=ccn2
		cci=cci2
		cpi=cpi2
...
function angle2k(df)
	...
	variable k0x, k1x, k2x, k3x,k0y, k1y, k2y, k3y
	if(cmpstr(mapp,"PolarAzimuth")==0)
		...
		ang2k(df,a0,b0,k0x,k0y); 		ang2k(df,a0,b1,k1x,k1y); 		ang2k(df,a1,b0,k2x,k2y); 		ang2k(df,a1,b1,k3x,k3y); 

//		kxmin=gmk*sin(a0*pi/180); kxmax=gmk*sin(a1*pi/180)	//min, max k before rotation
//		kymin=gmk*sin(b0*pi/180); kymax=gmk*sin(b1*pi/180)
		kxmin=min(k0x,k1x);	kxmax=max(k2x,k3x)
		kymin=min(k0y,k2y); kymax=max(k1y,k3y)
	...
	if(cmpstr(mapp,"PolarAzimuth")==0)
		...
		interp2d_w_fixed(ad0,polar,phi,kw)
	else
		string bs=(df+"beta_"),ts=(df+"theta")
		duplicate/o kw,$bs,$ts
		wave beta_=$bs, theta=$ts
		theta=asin(x/k)*180/pi
		beta_=asin(y/k/cos(asin(x/k)))*180/pi
		wavestats/q ad0
		interp2d_w_fixed(ad0,theta,beta_,kw)
	endif
	nvar rk=$(df+"recalcK")
	rk=0
	if (f0!=0)
...
function rotatek(df)
	...
	//kdtemp=kd(kxr)(kyr)
	interp2d_w_fixed(kd,kxr,kyr,kdtemp)	//** CHANGED BUT NOT TESTED **//
...
function k2angle(df)
	...
	string kws=(df+"kdata"), bs=(df+"beta_"),ts=(df+"theta")
...
static function almostEqual(a,b)
	variable a,b
...
function k2Ang(df,kx,ky,theta,beta_,phi)
	string df
	variable kx,ky
	variable &theta, &beta_,&phi
	variable k=0.5124*sqrt(gethv(df))
	svar mapp=$(df+"mapp")
	if (cmpstr(mapp,"TwoPolar")==0)
		theta=asin(kx/k)*180/pi
		beta_=asin(ky/k/cos(asin(kx/k)))*180/pi
...
function ang2k(df,theta,beta_,kx,ky)
	string df
	variable &kx,&ky
	variable theta, beta_
	variable k=0.5124*sqrt(gethv(df))
	kx=k*sin(pi/180*theta)
	ky=k*sin(pi/180*beta_)*cos(pi/180*theta)
	return 0
end
...
Window KPlotWin() : Graph
	...
	SetVariable ang0,limits={-Inf,Inf,0.02},value= root:KPlot:theta0
	SetVariable ang1,pos={5,41},size={100,15},proc=SetZeroAngles,title="beta0"
	SetVariable ang1,limits={-Inf,Inf,0.02},value= root:KPlot:beta0
	...
	SetVariable beta_,pos={298,33},size={70,15},title="b",font="Symbol",fSize=12
	SetVariable beta_,value= root:KPlot:vdbeta
	...
	PopupMenu cursorPopup,pos={459,50},size={75,20},proc=CursorMenuProc,title="Cursor..."
	PopupMenu cursorpopup,mode=0,value= #"\"  plain;  kgrid;-;set options...\""
EndMacro
...
//			zp=interp3d(w,axk,x,ayk)
		...
		if (kmap)
			...
			vdk=sqrt(ax^2 + ay^2)
			variable theta, beta_, phi
			k2ang(dfname,ax,ay,theta,beta_,phi)
			vdtheta=theta
			vdbeta=beta_
			vdphi=phi
		else
...
function doSymmetrize(d,name,symoption,whichside)
	...
	switch(symoption)	// numeric switch
		case 1:		// LR
			...
			string wx=(df+"twx"),wy=(df+"twy")
			duplicate/o w,$wx,$wy
			wave wvx=$wx, wvy=$wy
			wvx=x; wvy=sgn*sga*abs(x)
			interp2d_w_fixed(d,wvx,wvy,w)
			break						
		case 2:		//UD
		...
		case 3:		//LRUD
			...
			duplicate/o w,$wx,$wy
			wave wvx=$wx, wvy=$wy
			wvx=sgn*sga*abs(x); wvy=-sgn*sga*abs(y)
			interp2d_w_fixed(d,wvx,wvy,w)			
			break
		default:							
			duplicate/o d $name
...
function makeZWin(dfn,df)
	...
	variable w0=dimoffset(w,1), wd=dimdelta(w,1), ws=dimsize(w,1), w1=w0+(ws-1)*wd
...
//executes wvout=interp2d(w,x,y) without crashing
static function interp2d_fixed(w,wvout)
	wave w,wvout
	duplicate/o w temp_interp2d_fixed
	redimension/n=(-1,-1,3) temp_interp2d_fixed
	setscale/p z, 0,1,temp_interp2d_fixed
	temp_interp2d_fixed=w[p][q][0]
	wvout= interp3d(temp_interp2d_fixed,x,y,1)
	return 0
end

//executes wvout=interp2d(w,wx,wy) without crashing (wx,wy are waves)
static function interp2d_w_fixed(w,wx,wy,wvout)
	wave w,wx,wy,wvout //
	duplicate/o w temp_interp2d_fixed
	redimension/n=(-1,-1,3) temp_interp2d_fixed
	setscale/p z, 0,1,temp_interp2d_fixed
	temp_interp2d_fixed=w[p][q][0]
	wvout= interp3d(temp_interp2d_fixed,wx,wy,1)
	return 0
end


//executes wvout[][][i]=interp2d(w,x,y) without crashing
 function interp2d_i_fixed(w,wvout,i)
	wave w,wvout; variable i
	duplicate/o w temp_interp2d_fixed
	redimension/n=(-1,-1,3) temp_interp2d_fixed
	setscale/p z,0,1,temp_interp2d_fixed
	temp_interp2d_fixed=w[p][q][0]
	wvout[][][i]= interp3d(temp_interp2d_fixed,x,y,1)
	return 0
end