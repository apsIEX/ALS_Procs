			...
			if(matchOverlap)
				...
				if(i==0)
					...
					for (j=0; j<npm; j+=1)
						if(is3d)
							for(rr=hxzp[0];rr<=hxzp[10]; rr+=1)
								rght+=w[j+startpixel+numpixelsUse-1][rr][p]
							endfor
						else
							rght+=w[j+startpixel+numpixelsUse-1][p]
						endif
					endfor
					...
					for (j=0; j<npm; j+=1)
						if(is3d)
							for(rr=hxzp[0];rr<=hxzp[10]; rr+=1)
								lft+=w[startpixel-j][rr][i*numangle2+p]
							endfor
						else
							lft+=w[startpixel-j][i*numangle2+p]
						endif
					endfor
					nfyi=lft/rght
					smooth 200, nfyi
					...
					for (j=0; j<npm; j+=1)
						if(is3d)
							for(rr=hxzp[0];rr<=hxzp[10]; rr+=1)
								rght+=w[j+startpixel+numpixelsUse-1][rr][i*numangle2+p]
							endfor
						else
							rght+=w[j+startpixel+numpixelsUse-1][i*numangle2+p]
						endif
					endfor
				endif
				if(is3d)
...
Window KPlotWin() : Graph
	...
	Display /W=(618,78,1383,815) as "KPlot"
	...
	CheckBox kmap,pos={124,21},size={51,14},proc=Checkboxkmap,title="kmap?",value= 1
	SetVariable hv,pos={207,20},size={75,15},proc=Sethv,title="hv"
	SetVariable hv,value= root:KPlot:hv
	SetVariable theta0,pos={5,24},size={100,15},proc=SetZeroAngles,title="theta0"
	SetVariable theta0,limits={-Inf,Inf,0.1},value= root:KPlot:theta0
	SetVariable beta0,pos={5,41},size={100,15},proc=SetZeroAngles,title="beta0"
	...
	SetVariable phi0,pos={5,58},size={100,15},proc=SetZeroAngles,title="phi0"
