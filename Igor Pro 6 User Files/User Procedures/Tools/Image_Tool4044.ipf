...
//
#pragma version = 4.044
//v4.044	  1/11/10  AB load ct from igor user folder if avaliable
...
//#include "colortables"   //not needed if load ALL_CT in one shot
...
//===============
	...
	NewDataFolder/O/S $df
		...
		if (V_flag==0)
			...
			NewPath/q/z/o IDL_CT_path specialDirPath("Igor Pro User Files",0,0,0)+"User Procedures:ColorTables"
			if(V_flag!=0)        // If no color tables in user folder
				pathinfo Igor	// try igor procedures folder
				NewPath/q/o IDL_CT_path s_path+"User Procedures:ColorTables"
			endif
			LoadWave/T/Q/O/P=IDL_CT_path "IDL_CT.itx"	// -> ALL_CT
		endif
...
Function Image_Tool(df) : Graph
	...
	TitleBox version,pos={10,28},size={35,14},fsize=9, title="v4.043",frame=0, fstyle=2
