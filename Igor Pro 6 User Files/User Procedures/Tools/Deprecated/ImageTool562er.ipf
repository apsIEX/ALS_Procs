...
function tabproc(name,tab)	
	...
	setvariable setgamma,value=$(df+"gammaV"), disable=(tab!=2)	// We set the variable to support the old name
	...
	CheckBox setLinkCT, disable=(tab!=8)
...
function setupcontrols(df)
	...
	SetVariable setgamma,pos={74,y0+13},size={62,14},title="g"
	...
	SetVariable setgamma,limits={0.01,Inf,0.05},value=$(df+"gammaV")
	Slider slidegamma size={170,16},pos={66,55},ticks=0,vert=0,proc=GammaSliderProc
	Slider slidegamma value=0.23,limits={-1.5,1.5,0.01}, ticks=-5

//	groupBox Colors title="Color Options",pos={x0,y0}, size={90,60},fcolor=(65535,0,0),labelback=0
	PopupMenu SelectCT,pos={x0,y0+10},size={43,20},proc=SelectCTList,title="CT"
	PopupMenu SelectCT,mode=0,value= #"colornameslist()"
	checkbox invertCT,pos={x0+50,y0+13},size={80,14},title="Invert?"
...
//	checkbox invertCT,pos={153,45},size={80,14},title="Invert?"
	...
	CheckBox SetLinkCT pos={cstarth+2*cstep+bstep,vstarth},title=""
	execute "checkbox SetLinkCT,variable="+df+"LinkCT"
...
function setupV(dfn,w)
	...
	wave pmap=$(dF+"pmap")
	variable /g $(dF+"gammaV")
	nvar gammaV=$(dF+"gammaV")
	
	setformula $(df+"pmap") , "255*(p/255)^"+df+"gammaV)"
...
function /s marqueemenus()
	...
	if(strsearch(df,"ImageToolV",0)>0)
		string dfn=winname(0,1)
		nvar OverrideMarqueeMenu = $(df+"CustumMarqueeMenu")
//		if(CustumMarqueeMenu==0)
...
FUNCTION doImagetool5Graphmarquee()
	...
	strswitch(menustr)
		...
		case "Vert Expand":
		case "AutoScale":
			doMarqueeExpand(df , dfn,menustr)
			break
		case "Export":
			if(strlen(ImgName)>0)
				getimageinfo(df,imgname,s)
