...
function ROIupdatePolys(df)
	...
	setdrawlayer/k progfront
	setdrawenv fillpat=0,xcoord=bottom, ycoord=left,linethick=showROI, linefgc=(r,g,b)
	drawpoly/abs 0,0,1,1,imgrx,imgry
	setdrawenv fillpat=0,xcoord=bottom, ycoord=imgHL,linethick=hshowROI, linefgc=(r,g,b)
	drawpoly/abs 0,0,1,1,imgHrx,imgHry
	setdrawenv fillpat=0,xcoord=imgVB, ycoord=left,linethick=vshowROI, linefgc=(r,g,b)
	drawpoly/abs 0,0,1,1,imgVrx,imgVry
	if(ZTPmode)
		setdrawenv fillpat=0,xcoord=imgZTt, ycoord=imgZTr,linethick=ztShowROI, linefgc=(r,g,b)
...
static function adjustCTh(df)
	...
	nvar ict=$(df+"invertct"), im=$(df+"imgHmode")
	if((im==2)*(numpnts(imgrx)>0))
...
static function adjustCTv(df)
	...
	nvar ict=$(df+"invertct"),im=$(df+"imgVmode")
	if((im==2)*(numpnts(imgrx)>0))
...
static function adjustCTzt(df)
	...
	nvar ict=$(df+"invertct"),im=$(df+"imgZTmode"), ztpmode=$(df+"ztpmode")
	if(Ztpmode)
		if((im==2)*(numpnts(imgrx)>0))
