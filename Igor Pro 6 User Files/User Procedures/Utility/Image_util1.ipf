...
#pragma version=5.21
// 21/10/09 ab add rainbow light and purple-yellow color tables
// 5/5/09 ab -- make img_axis() static to prevent conflict
...
//#include "colortables"   //CT control bar works independently
...
// work on top image in top graph
	...
	//if (!DataFolderExists("root:colors:"))
		//string curr=GetDataFolder(1)
		//NewDataFolder/O/S root:colors
			make/o/n=(256,3) CT
			make/o/n=256 LUT
			string/G currCTname=CTnam, imCTwnam=imCTwn		//,CTnamelst
			variable/G currCTnum, LUTgamma=gamma, CTinvert=invert
	//SetDataFolder $curr
	...
	// set globals to top graph values values
	//   linear CT identity & load
	//SVAR CTnamelst=root:colors:CTnamelst			//s_colorNL
	//SVAR currCTname=root:colors:currCTname
	//NVAR currCTnum=root:colors:currCTnum
	//CTnamelst=CTlist_IDL()		//string function (versus extracting from string wave)
	//currCTname=CTnam
	currCTnum=WhichListItem( currCTname, CTnamelist(2),";",0 )+1		// -1 if not found
	currCTnum=Selectnumber( currCTnum==0, currCTnum, 1)		// not CT in list
		//print currCTnum
	CTselect("",currCTnum+1,"")		// load linear CT from file
	//execute "loadct(" +   num2str(currCTnum)    +    ")"
	// nonlinear gamma lookup table
	//NVAR LUTgamma=root:colors:LUTgamma
	//LUTgamma=gamma
	//CTinvert=invert
	//WAVE LUT=root:colors:LUT
	LUT=255*(p/255)^gamma
	// image index CT
	//SVAR imCTwnam=root:colors:imCTwnam
	//imCTwnam=imCTwn	
	//WAVE imCTw=$imCTwn
	...
	if (addctrl)
		...
		//PopupMenu CTpop value=root:colors:s_colorNL
		PopupMenu CTpop bodyWidth=120, fsize=8, proc=CTselect, mode=currCTnum+1  //global or local CTnum?
		PopupMenu CTpop value="Invert;"+CTnamelist(2), pos={5,2}
		Slider GammaSlide size={100,35}, vert=0, fsize=8, pos={130,2}
		Slider GammaSlide  proc=CTadjGamma , value=gamma  //global or local CTnum?
			//execute "Slider GammaSlide value="+num2str(gamma)
		Slider GammaSlide limits={0.05,SelectNumber(CTinvert==1, 20,2),0.05}
		ValDisplay GammaVal, bodyWidth=30,  fsize=9, title="g", font="Symbol" , pos={205,2}
		execute "ValDisplay GammaVal value="+num2str(gamma)
		//Checkbox Invert,  title="Invert", mode=0, pos={5,25}
		Checkbox Kill,  title="Kill", proc=CTkillControls, mode=1, pos={270,2}
		Checkbox CTrescale,  title="rescale CT (marquee)", mode=1, pos={5,24}, proc=CTrescale, value=0
		//NVAR KeepCTscale=root:colors:KeepCTscale
		Checkbox CTscale,  title="Keep CT scale", mode=0, pos={270,20}
	else
...
//=======
	...
	if (mode==0)   // std numbering with space
		...
		lst+="36. Volcano;37. Waves;38. Rainbow18;39. Rainbow + white;40. Rainbow + black;41. Rainbow light;42. Purple-Yellow;"
	elseif (mode==1)   //2-digit numbering with no spaces
		...
		lst+="Volcano;Waves;Rainbow18;Rainbow + white;Rainbow + black;Rainbow light;Purple-Yellow"
	endif
	return lst
End

Function CTselect(ctrlName,popNum,popStr) : PopupMenuControl
//==================
	String ctrlName
	Variable popNum
	String popStr
	...
	case 1:			//Invert
		...
		SetDataFolder $curr
		 ...
		 Slider GammaSlide value=LUTgamma
		 Slider GammaSlide limits={0.05,SelectNumber(CTinvert==1, 20,2),0.05}
	...
	default:
		...
		string fname=StringFromList(currCTnum-1, CTnamelist(1)) 
		//print fname
...
//==============
	...
	SVAR imCTwnam=imCTwnam  //root:colors:imCTwnam
	NVAR LUTgamma=LUTgamma, CTinvert=CTinvert //root:colors:LUTgamma, CTinvert=root:colors:CTinvert
	//variable gamma=sliderValue
	SetDataFolder $curr
	LUTgamma=sliderValue

	//if(event %& 0x1)	// bit 0, value set
		WAVE imCTw=$imCTwnam				//after return to current folder
		LUT=255*(p/255)^LUTgamma
		//LUT=255*(p/255)^Selectnumber(CTinvert==1, 1/LUTgamma, LUTgamma)
		imCTw=CT[ LUT[p] ][q]
		execute "ValDisplay GammaVal value="+num2str(LUTgamma)
		CTwriteNote(imCTw, currCTname, LUTgamma, CTinvert)
	//endif
...
Function/S CTwriteNote(CTw, CTnam, gamma, invert)
//=============
	WAVE CTw
	string CTnam			
	variable gamma, invert
	...
	string notestr, CTlst
	CTlst="name="+CTnam+",gamma="+num2str(gamma)+",invert="+num2str(invert)
	notestr=note(CTw)
...
// make IDL index Color Table from scratch
	...
	WAVE imCTw=$imCTwn
	//variable gammap=SelectNumber(invert==1, gama, 1/gama )
	imCTw=CT[ 255*(p/255)^gama ][q]
	CTsetScale(imCTw, dmin, dmax, invert)	
	CTwriteNote(imCTw, CTnam, gama, invert)
