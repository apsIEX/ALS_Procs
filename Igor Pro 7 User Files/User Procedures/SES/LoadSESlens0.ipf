...
// optional reinterpolate to scaled array(s)
	if (escal==1)	
		...
		Duplicate/O  root:LENS:hv $(base+"_x")
	endif	
...
// saves values in root:LENS folder variables
	...
	SVAR sample=sample, slits=slits, comment=comment, basenam=basenam
	...
	Open/R file as filpath+filnam
		...
		variable filetype=1.1, nheaderlines=1
		IF (stringmatch( sline, "*LENS 1.3*"))
			filetype=1.3; nheaderlines=14
		endif
		IF (stringmatch( sline, "*LENS 1.4*"))
			filetype=1.4; nheaderlines=17
		endif
		//print sline, stringmatch( sline, "*LENS 1.3*" )
		IF (filetype>=1.3)
			hdrlst=""
			FReadLine file, sline		//skip line
			ii=0
			DO
				FReadLine file, sline
				hdrlst+=sline
				ii+=1
			WHILE( ii<nheaderlines)
			Close file
			
			//nchan=str2num( StringByKey("Number of Channels", hdrlst, "=", "\r") )
			sample=StringByKey("Sample", hdrlst, "=", "\r")
			slits=StringByKey("Slits", hdrlst, "=", "\r")
			comment=StringByKey("Comment", hdrlst, "=", "\r")
			hv_start=str2num( StringByKey("Start", hdrlst, "=", ",") )
			hv_end=str2num( StringByKey("Stop", hdrlst, "=", ",") )
			hv_inc=str2num( StringByKey("Step", hdrlst, "=", ",") )
			dwell=str2num( StringByKey("Dwell", hdrlst, "=", ",") )
			
			if (filetype==1.4)
				date_=StringByKey("Date", hdrlst, "=", "\r")
				endtime=StringByKey("End Time", hdrlst, "=", "\r")
				grating=StringByKey("Grating", hdrlst, "=", "\r")
				harmonic=str2num( StringByKey("Harm", hdrlst, "=", ",") )
				// also endtime
			else
				date_=" "
				endtime=" "
		...
		ELSE
			...
			//Read and interpret #channels line
			FReadLine file, sline
			//print sline
			nchan=str2num( StringByKey("Number of used channels", sline, " ") )
			...
			DO
				FReadLine file, sline
				//print sline
				if (stringmatch(sline, "*Comment:*")==1)
					break
				endif				
				ii+=1
			WHILE( ii<5 )
			...
			// Read comment line
			FReadLine file, sline
			comment=sline[0, strlen(sline)-2]		//strip CRLF from PC
			sample="sample"
			slits="slits"
			
			// Find region line; otherwise append to comment
			ii=0
			DO
				FReadLine file, sline
				//print sline
				if (stringmatch(sline, "*Region*")==1)
					break
				endif
				comment+=" / "+sline[0, strlen(sline)-2]			
				ii+=1
			WHILE( ii<4 )
			
			//Interpret parameter line
			//sline=sline[0, strlen(sline)-2]	
			//print sline
			hv_start=str2num( StringByKey("Startvalue", sline, " ", " ") )
			hv_end=str2num( StringByKey("Stopvalue", sline, " ", " ") )
			hv_inc=str2num( StringByKey("Stepsize", sline, " ", " ") )
			dwell=str2num( StringByKey("Counttime", sline, " ", " ") )
			
		Close file
	ENDIF
	
	// put specific values into string infowav 
	make/o/T/N=11 infowav
	infowav[0]=ExtractNameLENS( filnam, 1, 0 )
	infowav[1]=sample
	infowav[2]=slits
	infowav[3]=grating	
	infowav[4]=num2str( hv_start )
	infowav[5]=num2str(hv_end )
	infowav[6]=num2str( hv_inc )
	infowav[7]=num2str( dwell )
	infowav[8]=date_
	infowav[9]=endtime	
	infowav[10]=comment[0,99]
...
//-----------------
	...
	if (V_flag==0)
		...
		NewDataFolder/O/S root:LENS
			...
			make/o/n=6/T chanlistw:=StringFromList( p, chanlist, ",")
			make/o/n=50 hv=p, chan0, chan1, chan2, chan3, chan4, chan5, chan6, chan7, normdat
		SetDataFolder root:
...
Window Load_LENS_Panel() : Graph
	...
	String fldrSav= GetDataFolder(1)
	SetDataFolder root:LENS:
	Display /W=(633,58,865,505) normdat vs hv as "Load_LENS_Panel"
	SetDataFolder fldrSav
	...
	PopupMenu popup_file,pos={7,43},size={129,19},proc=SelectFileLENS,title="File"
	PopupMenu popup_file,mode=3,popvalue="uge_003.usc",value= #"root:LENS:filelist"
	Button FileUpdate,pos={156,45},size={55,16},proc=UpdateFolderLENS,title="Update"
	SetVariable val_comment,pos={12,84},size={199,14},title=" comment",fSize=9
	...
	PopupMenu popup_norm,pos={101,198},size={72,19},proc=SetInputPopLENS,title="Norm"
	PopupMenu popup_norm,mode=2,popvalue="0",value= #"\"None;0;1;2;2;3;4;5\""
	PopupMenu popup_plot,pos={20,199},size={62,19},proc=SetInputPopLENS,title="Plot"
	PopupMenu popup_plot,mode=2,popvalue="1",value= #"\"0;1;2;3;4;5\""
	...
	SetVariable val_sample,pos={14,67},size={110,14},title="sample",fSize=9
	SetVariable val_sample,limits={-Inf,Inf,1},value= root:LENS:sample
	SetVariable val_slits,pos={130,66},size={80,14},title=" slits",fSize=9
	...
	SetDrawEnv save
	SetDrawEnv fsize= 10
	DrawText -0.204615622105588,-0.105263157894737,"v 1.5"
	DrawText 300,17,"v 1.5"
...
Window RawLENS_5() : Graph
	...
	AppendToGraph/L=ch2 chan2 vs hv
	AppendToGraph/L=ch3 chan3 vs hv
	AppendToGraph/L=ch4 chan4 vs hv
	SetDataFolder fldrSav
	ModifyGraph grid(bottom)=1
	ModifyGraph tick(left)=2,tick(bottom)=2
	ModifyGraph mirror=2
