...
//v2.04 5/09 ER stripped extra return chars from wavenote
//v2.05 6/9/2013 ER fixed missing comma for igor 6.3
//v2.06 ER changed lines to make compatible with igor 7.04, didn't test to see if worked
//    see the lines marked //ER 2.06
		...
		for (i=i0; i<=i1; i+=1)
			for (j=j0; j<=j1; j+=1)
				for(k=0; k<numMir; k+=1)
					//print i,j,k
					img2[i][j]+=isvalid[i][j][k]	//add 1 if valid number, zero if otherwise  //ER 2.06 i, j on RHS used to be p, q
					if (isvalid[i][j][k])
	...
	for(zp=zp0; zp<=zp1; zp+=1)
		...
		for (i=i0; i<=i1; i+=1)
			for (j=j0; j<=j1; j+=1)
				for(k=1; k<ns; k+=1)
					//print i,j,k
					img2[i][j]+=isvalid[i][j][k]	//add 1 if valid number, zero if otherwise  //ER 2.06
					if (isvalid[i][j][k])
						//rotated data is valid, so average it in
						if(numtype(img[i][j][zp])==0)
							//unrotated data is valid, average it with rotated data
...
macro theta2k(wv)
	...
	string lwlvnm=stringbykey("LWLVNM",replacestring(num2char(13),note($wv),""),"=")	//strip end of line character
	...
	if(cmpstr(lwlvnm[0,4], "ky-kz")==0)
		...
		if(ndims==3) 
			...
			variable monoev=numberbykey("MONOEV",replacestring(num2char(13),note($wv),""),"=")
