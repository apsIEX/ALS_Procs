...
proc MakeDispersionCorrection(wv, method, cOut)
	...
	if(method==1) //trapezoidal
		...
		variable/g xL0:=tx[1]-1/sL*ty[1]				//left line intersects y=0
		variable/g xR0:=tx[2]-1/sR*ty[2]				//right line intersects y=0
		variable/g xL1:=tx[1]+1/sL*(dy-ty[1]-1)	//left line intersects y=ny-1	
		variable/g xR1:=tx[2]+1/sR*(dy-ty[2]-1)	//right line intersects y=ny-1
		make/o/n=(ny) stretch
...
function GetParms(ctrlName):ButtonControl
	...
	nvar ss=$(df+"slitSpacing"), dd=$(df+"distance")
	variable sspacing=ss, dist=dd
	prompt sspacing, "Slit Spacing"
	prompt dist, "Distance to sample"
	//doGetParms(df,sspacing, dist,,)
	doprompt "Enter Parameters", sspacing, dist
	if (v_flag)
		return -1
	endif
	ss=sspacing
...
Function doFindPeaks(ctrlName) : ButtonControl
	...
	nvar y0=$(df+"y0"), dy=$(df+"dy")
	duplicate/o/r=[][200] testdetsource $(df+"lineout")
	wave lo=$(df+"lineout")
	...
	nvar slitspacing=$(df+"slitSpacing"), distance=$(df+"distance")
	setscale/i x, -16, 16, "angle, deg" testdet
	testdet=interp2d(testdetsource, interp2d(cx, y, distance/slitSpacing*tan(x*pi/180)), y+interp(interp2d(cx, y, distance/slitSpacing*tan(x*pi/180)), ex, ey)-getavg(ey))
...
function ApplyDispersionCorrectionF(dat,cwv,ovw,newin)
	...
	nvar xavg=$(dF+"xavg"), distance=$(df+"distance"), slitspacing=$(df+"slitspacing")
	wave ex=$(df+"ex"), ey=$(df+"ey"), stretch=$(df+"stretch")
	wave cx=$(df+"cx")
	...
	setscale/i x -16,16,"angle",c_temp
	duplicate/o c_temp c_tempx c_tempy

	if(method==1)	//trapezoid
		c_tempx=(x-xavg*(1-stretch(y)))/stretch(y)
	...
	if(dim3d)
		...
		do
			print k
			c_temp0=dat[p][q][k]
			c_temp=interp2d(c_temp0, c_tempx, c_tempy)
			if(ovw)
				//dat[][][k]=c_temp[p][q]
		...
		duplicate/o dat xxx
		setscale/i x -15.5,15.5,"angle",xxx
