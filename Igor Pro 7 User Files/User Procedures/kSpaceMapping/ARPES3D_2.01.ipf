...
proc MakeDispersionCorrection(wv, method, cOut)
	...
	if(method==1) //trapezoidal
		...
		testdet:=interp2d(testDetSource, (x-xavg1*(1-stretch(y)))/stretch(y)+shift(y),y+interp(x,ex,ey)-getavg(ey))
	else
...
function ApplyDispersionCorrectionF(dat,cwv,ovw,newin)
	...
	nvar xavg1=$(dF+"xavg1"), distance=$(df+"distance"), slitspacing=$(df+"slitspacing")
