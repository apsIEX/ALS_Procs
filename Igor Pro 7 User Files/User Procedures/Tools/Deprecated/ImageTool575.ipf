...
#include "colorTables"
	static constant MAJOR_VERSION = 5.73
	...
	//			 fixed dest folder of convolution wave
	//5.71 ER added display of integrated data at cursors using bin range
	//			 fixed bug setting range of "x0" and "x1" norm ranges which get set backwards if xwave is monotonically decreasing
	//5.71 AB Add note to exported wave to list source.
	//5.72 Add textbox with wave name for exported graphs
	//		 hold cmd to move cursor with scroll wheel 	
	//5.73 	Stack panels update without comming to front
	//		 Make Extract Lineout work for multiple segments
	//5.75 ER fix missing commas for igor 6.32
...
function tabproc(name,tab)	
	...
	valdisplay valDi,disable=(tab!=0)
...
function setupcontrols(df)
	...
	ValDisplay valDi,pos={cstarth+4*cstep,vstarth+20},size={91,14},title="Di"
...
function setupV(dfn,w)
	...
	wave/t sarr=$(df+"sarr")
	sarr={"xp","yp","zp","tp"}; 	execute wv4di(w,df,"d0",sarr,dnum)   //;print wv4di(w,df,"d0",sarr,dnum)
