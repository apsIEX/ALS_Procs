...
function setupcontrols(df)
	...
	setvariable setXP,pos={67,41},size={70,14},proc=SetHairXY,title="XP"
	...
	SetVariable setY0,limits={-6.00024,12,0.031972},value= $(df+"yc")
	SetVariable setYP,pos={141,41},size={70,14},proc=SetHairXY,title="YP"
...
function setupV(dfn,w)
	...
	wave imgh=$(df+"imgh");	setscale/p x,x0,xd,xl,imgh;		setscale/p z,z0,zd,zl,imgh
	...
	//execute wv4d(w,df,"d0","xp","yp","zp","tp")
	//execute wv4d(w,df,"img","p","q","zp","tp")
	//execute wv4d(w,df,"imgh","p","yp","q","tp")
	//execute wv4d(w,df,"imgv","xp","q","p","tp")
	//execute wv4d(w,df,"px","p","yp","zp","tp")
	//execute wv4d(w,df,"pyx","xp","p","zp","tp")
	//execute wv4d(w,df,"pz","xp","yp","p","tp")
	//execute wv4d(w,df,"pt","xp","yp","zp","p")
	//execute wv4d(w,df,"imgzt","xp","yp","p","q")
	pyy=y0+p*yd
...
function img4DHookFcn(s)
	...
	variable xcold,ycold,zcold,tcold
	...
	if(strsearch(s,"EVENT:mouse",0)>0)
		...
		if(modif==9)	//9 means "1001" = cmd/ctrl + mousedown
			...
			if((xc==xcold)*(yc==ycold))
				//must be on zc or tc axis or zt image
				if(ztpmode)
					...
					if(hasZP)
						...
						zc=selectnumber((zx>zxmin)*(zx<zxmax)*(zy>zymin)*(zy<zymax),zc,zx)	
					endif
					if((zcold==zc)*(hasTP))
