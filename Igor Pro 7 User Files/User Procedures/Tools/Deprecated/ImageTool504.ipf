...
static function getleft(df,which)
...
static function getwid(df,which)
	string df		//data folder
	variable which	//1=x, 2=y, 3=z, 4=t
	wave img=$(df+"img"), pz=$(df+"pz"), pt=$(df+"pt")
	switch(which)
		case 1:
			return dimdelta(img,0)*dimsize(img,0)
			break
		case 2:
			return dimdelta(img,1)*dimsize(img,1)
		case 3:
			return dimdelta(pz,0)*dimsize(pz,0)
		case 4:
			return dimdelta(pt,0)*dimsize(pt,0)
			break
	endswitch
end
