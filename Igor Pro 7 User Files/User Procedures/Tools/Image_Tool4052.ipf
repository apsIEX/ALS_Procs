...
//	variable/C coffset=GetWaveOffset(root:IMG:HairY0)
	...
	if ((cmpstr(popStr,"Find Edge")==0) + (cmpstr(popStr,"Fit Edge")==0))
		...
		Variable fitedge=NumVarOrDefault(df+"edgefit", 2-(cmpstr(popStr,"Find Edge")==0) )
...
// now handle subfolders
	...
	if(datafolderexists(df))
		...
		do	
			...
			variable ii
			for(ii=0;ii<numdf;ii+=1)
				KillAllinFolder(df+GetIndexedObjName(df, 4, ii ))
			endfor
			count = CountObjects(df,1)
			count +=  CountObjects(df,2)
			count +=  CountObjects(df,3)
			count +=  CountObjects(df,4)
			i+=1
		   doupdate // this seems to be necissary to clear out some dependencies
		while(count*(i<10))
