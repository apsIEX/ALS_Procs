...
//1.12 ER 2016-08-11 made fit spectra faster (cut down history overhead)
...
//ER 2016.08.08 commented out line see //IGOR 7 where it can;t compile, I don't know what the intent was
...
proc doLogoWindow()
	...
	dowindow/f PFLogo
	if (v_flag==0)
		if (exists("root:PF:logo")==0)
			pathinfo igor
			NewPath/q/o PFPath s_path+"User Procedures:PeakFitter"
			ImageLoad/P=PFPath/T=tiff/N=logo/o "logo.tif"
			movewave logo root:PF:
			movewave logocmap root:PF:
		endif
		display
		DoWindow/C/T PFLogo,"PFLogo"
		appendimage root:PF:logo
		ModifyImage logo cindex= root:PF:logoCMap
		setaxis/a/r left
		ModifyGraph width=906/2,height=348/2,nticks=0, wbrgb=(65535,49157,16385)
		TextBox/C/N=text0 "\JCPeak Fitter Version "+root:PF:VERSION+"\r(c) 2002 Eli Rotenberg, Lawrence Berkeley National Laboratory";DelayUpdate
...
proc getWaves(yw)
	string yw
	prompt yw,"y-wave",popup wavelist("*",";","WIN:"+root:PF:winnam)
	silent 1; pauseupdate
	...
	if(wavedims($yw)==2)
		//wave is a matrix
		if(dimsize($yw,1)==1)
			root:PF:isSeries=0	//just a trivial one-column matrix
		else
			root:PF:isSeries=1	//series fit
		endif
	else
		root:PF:isSeries=0
	endif
	string/g root:PF:ywv=yw, root:PF:xwv=xwavename(winnam,ywv)
	variable/g root:PF:nchan
	if(root:PF:isSeries)
	 	string/g root:PF:axName="LChannel" //name for axis for spectral data
	 	string/g root:PF:g_sd=yw
		root:PF:xwv=xwavename(winnam, ywv)  //ywv+"_x"
		root:PF:nchan=dimsize($yw,1)
		root:PF:ywv="root:PF:SingleChannel"
		make/o/n=(dimsize($root:PF:g_sd,0)) root:PF:SingleChannel
		string ss=waveinfo($root:PF:g_sd,0)
		SetScale/P x dimoffset($root:PF:g_sd,0),dimdelta($root:PF:g_sd,0),"", root:PF:singlechannel
		string/g root:PF:g_indname, root:PF:g_indunits
		variable/g root:PF:npnts=np
		variable/g root:PF:g_indstart=dimoffset($g_sd,1), root:PF:g_indincr=dimdelta($g_sd,1)
		//initializeSeries(yw,,,,)
		//setdatafolder ::
		//dowindow/f $root:PF:winnam
		//append/l=$root:PF:axname root:PF:singlechannel vs $(root:PF:xwv)
		appwv(root:PF:xwv,"root:PF:singlechannel",root:PF:winnam)
		ModifyGraph/w=$root:PF:winnam mode(SingleChannel)=3,marker(SingleChannel)=18,msize(SingleChannel)=2
		ModifyGraph/w=$root:PF:winnam axisEnab(LChannel)={0.55,1},freePos(LChannel)=0
		SetVariable channel,pos={ 100,0},size={100,14},proc=SetVarChannel,title="channel",win=$root:PF:winnam
		SetVariable channel,fSize=9,limits={0,root:PF:nchan-1,1},value= root:PF:channel,win=$root:PF:winnam
		Button showFitter,pos={215,0},size={50,20},title="FitPanel",proc=fitterwindowproc,win=$root:PF:winnam
		Button showHistory,pos={270,0},size={50,20},proc=HistoryWindowProc,title="History",win=$root:PF:winnam
		variable/g root:PF:whichHistBrowse=0
		variable startChan=round(root:PF:nchan/2)
		
		string dwv="Singlechannel"
 		make/o/n=(root:PF:nchan) root:PF:hist_npks; root:PF:hist_npks=nan
		make/o/n=(root:PF:nchan) root:PF:hist_chisq; root:PF:hist_chisq=nan
		make/o/n=(root:PF:nchan) root:PF:hist_isStored; root:PF:hist_isStored=0
 		make/o/n=(20,root:PF:nchan) root:PF:hist_fittype; root:PF:hist_fittype=nan
	 	make/o/n=(7,20,root:PF:nchan) root:PF:hist_fitparms; root:PF:hist_fitparms=nan
 		make/o/n=(7,20,3,root:PF:nchan) root:PF:hist_hold
		make/o/n=(root:PF:nchan+1,20) root:PF:hist_POS, root:PF:hist_AMP, root:PF:hist_LW
		make/o/n=(root:PF:nchan+1,20) root:PF:hist_GW, root:PF:hist_ASYM, root:PF:hist_SPLIT, root:PF:hist_RATIO
		make/o/n=(root:PF:nchan+1,20) root:PF:hist_indvar
		root:PF:hist_indvar[][]=p*g_indincr+g_indstart
		make/o/n=(root:PF:nchan+1,20) root:PF:browse_y, root:PF:browse_x
		
		ModifyGraph/w=$root:PF:winnam axisEnab(left)={0,0.45}
		//make cursor line and enable live update
		make/o/n=2 root:PF:csrx root:PF:csry
		root:PF:csrx={-inf,inf}
		root:PF:csry={hist_indvar[startchan][0],hist_indvar[startchan][0]}
		appwvleft("root:PF:csrx", "root:PF:csry", root:PF:winnam)
		ModifyGraph/w=$root:PF:winnam rgb(csry)=(0,65535,65535)
		SetWindow $root:PF:winnam hook=imgFitHookFcn, hookevents=3
		copychannel(startChan)
		
		//append position history
		root:PF:hist_pos=nan
		
		appwvleft("root:PF:hist_POS", "root:PF:hist_indvar", root:PF:winnam)
		ModifyGraph mode(hist_indvar)=3,marker(hist_indvar)=8,msize(hist_indvar)=2;DelayUpdate
		ModifyGraph rgb(hist_indvar)=(65535,0,0)
	else
 		string/g root:PF:axName="Left"
		root:PF:nchan=1
		variable npnp=numpnts($root:PF:ywv)
		root:PF:npnts=npnp
	endif
	dowindow/f $root:PF:winnam
	if (root:PF:isSeries==0)
		modify rgb($root:PF:ywv)=(0,0,0)
	else
...
//  Modifier bits:  0001=mousedown, 0010=shift  , 0100=option/alt, 1000=cmd/ctrl
	...
	svar wn=root:PF:winnam
	wave hi=root:PF:hist_indvar, csry=root:PF:csry
	nvar nc=root:PF:nchan, ch=root:PF:channel
	nvar auto=root:PF:chanAutoHist
...
function copyChannel(which)
	...
	svar ss=root:PF:g_sd//, sx=root:PF:xwv
	wave sc=root:PF:SingleChannel
	wave gy=$ss//, scx=$sx
	sc=gy[p+which*dimsize(gy,0)]    //  gy[p+which*numpnts(scx)]
	nvar chan=root:PF:channel
	chan=which
	wave csry=root:PF:csry, hi=root:PF:hist_indvar
...
proc updateXPSFit(pp)
	variable pp
	silent 1;pauseupdate
	variable FEPos=root:PF:FEPos , FEWidth=root:PF:FEWidth , hasFE=root:PF:hasFE
	makeconstraints()
	//variable dummy=setupPkTyp()
	variable hasBkgd
	variable/g v_fititerstart
	wavestats/q root:PF:bkgdfit
	hasBkgd=1//(v_numNans==0)
	root:PF:peakfit=0
	string peakname
	//dowindow/f $root:PF:winnam
	string wn=root:PF:winnam
	if(hasBkgd)
		if (strlen(root:PF:xwv)==0)
			root:PF:bkgdfit=BkgdFunction(fitparms,fittype,x)
		else
			root:PF:bkgdfit=BkgdFunction(fitparms,fittype,::$xwv)
		endif
	endif
	iterate(20-root:PF:numpks)
	...
	if(root:PF:numpks==0)
		...
		iterate(root:PF:numpks)
			peakname="root:PF:peak"+num2str(i+1)
			if((pp==0)+((i+1)==pp)) then
				if (strlen(root:PF:xwv)==0)
			...
			root:PF:peakfit+=$("peak"+num2str(i+1))
			if(hasBkgd) then
				root:PF:peakfit-=bkgdfit
				appwv(root:PF:xwv,"root:PF:bkgdfit",wn)
			endif
			appwv(root:PF:xwv,peakname,wn)
		loop
		...
		if (hasFE)
			root:PF:peakfit*=( 1/(exp((x-FEPos)/FEwidth)+1) )
		endif
		appwv(root:PF:xwv,"root:PF:peakfit",wn)
	endif
	string resname="res_"+ywvnm(0)
	if (exists(resname)==1)		
		$resname= $(ywvnm(1)) -  root:PF:peakfit
	endif
end
...
function/s BkgdHoldString()
	string h=""
	nvar b0=root:PF:bkgd0,b1=root:PF:bkgd1,b2=root:PF:bkgd2,b3=root:PF:bkgd3,b4=root:PF:bkgd4, b5=root:PF:bkgd5,b6=root:PF:bkgd6
	wave hold=root:PF:hold
	NVAR hasFE = root:PF:hasFE
...
function/s HoldString()
	string h=""
	variable i=0,j
	wave fp=root:PF:fitparms, hold=root:PF:hold
	nvar npks=root:PF:numpks
	//background
	j=1
	do
...
//should not be in PF
function setupPkTyp()
	nvar numpks=root:PF:numpks
	wave fitparms=root:PF:fitparms, fittype=root:PF:fittype
	make/o/n=7 root:PF:nantemp
	wave nantemp=root:PF:nantemp
	variable hasSO,i=1
	if (numpks>=1)
		do
...
function XPSFit(fitparms,fittype,xx)
	...
	variable xx
	variable res=0,i
	nvar hasbkgd=root:PF:hasbkgd, np=root:PF:numpks
	svar xwvnm=root:PF:xwv
	wave xwv=$xwvnm
	if(hasBkgd)
		if (strlen(xwvnm)==0)
			res=BkgdFunction(fitparms,fittype,xx)
		else
			//res=BkgdFunction(fitparms,fittype,xwv)  //IGOR 7
		endif
	endif
	i=0
	do
...
proc testTime()
	...
	iterate(20)
		//print i
		PF=xpsfit(root:PF:fitparms,root:PF:fittype,t226_x)
	loop
...
function fitFunction(ww,xx)
	wave ww; variable xx
	wave ft=root:PF:fittype
	wave fp=root:PF:fitparms
	svar xwvname=root:PF:xwv
	wave xwv=$xwvname
	svar ywvname=root:PF:ywv
	wave ywv=$ywvname
	nvar npks=root:PF:numpks
	nvar FEPos=root:PF:FEPos , FEWidth=root:PF:FEWidth , hasFE=root:PF:hasFE
	wave convStore=root:PF:convStore
	...
	if(cp==0)	//only executes once per iteration
		make/o/n=(npks+1) root:PF:hold1store
		make/o/n=(nparms+1,npks+1) root:PF:Amp0Store
		//print npts,npks+1,nparms+1
		make/o/n=(npts,npks+1,nparms+1) root:PF:convStore
		if(strlen(xwvname)==0)
			SetScale/P x dimoffset(ywv,0),dimdelta(ywv,0),"", convstore
		endif
		//print npts,npks,nparms
	endif
	wave hold1Store=root:PF:hold1store
	wave hold=root:PF:hold
	wave amp0store=root:PF:amp0store
	...
	wave ftft=root:PF:fittype
	if((cp<=nparms))
		//precalculate amplitudes for timesaver (3)
		amp0store[cp][1,npks]=subfunction0(ww,root:PF:fittype,q)
		i=1
		do
			if(abs(ft[i])==4)
				if (strlen(xwvname)==0)
					convstore[][i][cp]=subfunction(ww,root:PF:fittype,x,i)
				else
					convstore[][i][cp]=subfunction(ww,root:PF:fittype,xwv[p],i)
				endif
			endif
			i+=1
		while(i<=npks)
	...
	do
		...
		if(isSameX*(holdIs1+parmIsNaN+fittingOtherPeak))	//timesavers (1) and (2)
			...
			if(i==0) 
				res=bkgdfunction(ww,root:PF:fittype,xx)
			else
				if(abs(ft[i])==4)
					res=convStore[floor(cp/(nparms+1))][i][evwithinX]				//timesaver (4)
				else
					res=subfunction(ww,root:PF:fittype,xx,i)
				endif
...
function addBkRg(type)
	...
	svar wn=root:PF:winnam,an=root:PF:axName
	...
	svar ywv=root:PF:ywv,xwv=root:PF:xwv
	wave bk=root:PF:bkgdrgn, bw=root:PF:bkgdweight
	wave yw=$ywv
	if(strlen(xwv))
		findlevel/q $xwv,x0
		p0=v_levelx
	...
	execute "appwv(root:PF:xwv,\"root:PF:bkgdrgn\",root:PF:winnam)"
...
proc appwv(xw,yw,wn)
	...
	if (strsearch(ss,ywt+";",0)<0) 
		if (strlen(xw)==0)
			appendtograph/w=$wn/l=$root:PF:axname $yw
		else
			appendtograph/w=$wn/l=$root:PF:axname $yw vs $xw
		endif
	endif	
end
...
//	execute "doCheckCursors()"
	svar wn=root:PF:winNam,ywv=root:PF:ywv
	nvar np=root:PF:npnts
...
proc doCheckCursors()
	variable npA=numpnts(csrWaveRef(A,root:PF:winNam)),npB=numpnts(csrWaveRef(B,root:PF:winNam))
	variable ans=((npA==root:PF:npnts) * (npB==root:PF:npnts))
	if (ans)
		return 1
	else
		doAlert  0, "Sorry, you must have 2 cursors on '"+root:PF:ywv+"' or a wave like it"
		abort
		return 0
	endif
...
//should not be in root:PF
proc doBkgdGuess()
	variable nterms=root:PF:fittype[0]
	appwv(root:PF:xwv,"root:PF:bkgdfit",root:PF:winnam)
	ModifyGraph lstyle(bkgdFit)=1,rgb(bkgdFit)=(0,0,65535)
	variable hasX=strlen(root:PF:xwv) >0
	if (nterms>1)
		string cmd="CurveFit "
		if (nterms==2)
			cmd +="line,"
		...
		cmd+="root:PF:bkgdrgn /w=root:PF:bkgdweight "
		if (hasX)
			cmd += "/x="+root:PF:xwv
		endif
		execute cmd
		if(hasX) 
			duplicate/o $(root:PF:xwv) root:PF:tempX	   //get around igor limitation in next line(s)
			root:PF:bkgdfit=polyflex(::w_coef,tempX)
		else
			root:PF:bkgdfit=polyflex(::w_coef,x)
		endif
		root:PF:fitparms[0,numpnts(w_coef)-1][0]=::w_coef
		if(numpnts(w_coef)<=6)
			root:PF:fitparms[numpnts(w_coef),][0]=nan
		endif
	else		//offset only; just average the points
		wavestats/q root:PF:bkgdrgn
		variable temp=v_avg
		root:PF:bkgdfit=temp
		root:PF:fitparms[0][0]=temp
		root:PF:fitparms[1,][0]=nan
	endif
	setdatafolder PF
	updatePFPanel()
	setdatafolder ::
...
proc doKillPk()
	if (root:PF:numpks >0)
		doalert  1, "Are you sure you want to kill peak #"+num2str(root:PF:peak)+"?"
		if (v_flag==1)
			DeletePoints/M=1 root:PF:peak,1, root:PF:fitparms
			DeletePoints/M=1 root:PF:peak,1, root:PF:hold
			DeletePoints/M=1 root:PF:peak,1, root:PF:minmax
			DeletePoints root:PF:peak,1, root:PF:fittype
			remwv("peak"+num2str(root:PF:numpks),root:PF:winnam)
			root:PF:numpks-=1
...
proc doNewPeak(type)
	...
	root:PF:numpks+=1
	redimension/n=(7,root:PF:numpks+1) root:PF:fitparms
	redimension/n=(7,root:PF:numpks+1,3) root:PF:hold
	redimension/n=(7,root:PF:numpks+1,2) root:PF:minmax
	redimension/n=(root:PF:numpks+1) root:PF:fittype
	//----make some default parameters for relative peak constraint
	print root:PF:numpks
	if(root:PF:numpks==1)
		root:PF:hold[][root:PF:numpks][2]=2  //default peak 1 is relative to peak 2
	else
		root:PF:hold[][root:PF:numpks][2]=numpks-1
	endif
	root:PF:hold[0][root:PF:numpks][1]=0
	root:PF:hold[1,6][root:PF:numpks][1]=1
	//----
	root:PF:minmax[][root:PF:numpks][]=nan
	root:PF:peak=numpks
	if (cmpstr(type,"CSR")==0)    //csrs on center, center+halfwidth
		doThisPeak(hcsr(a),hcsr(b))
	else
		getmarquee/k $root:PF:axname,bottom //marquee on entire peak (lower left to lower right)
...
function doThisPeak(ha,hb)
	...
	silent 1;pauseupdate
	execute "askPeakType()"
	nvar pkty=root:PF:nextPkAdd
	nvar numpks=root:PF:numpks
	if (!numpks)
		abort "Cannot execute because no current peak exists"
	endif
	wave fp=root:PF:fitparms, ft=root:PF:fittype, bkgdfit=root:PF:bkgdfit
	nvar peak=root:PF:peak, lwg=root:PF:LWGuess, ag=root:PF:ASYMguess, sg=root:PF:splitguess, rg=root:PF:ratioGuess
	fp[0][peak]=ha
	svar ywv=root:PF:ywv								//POS
	wave yw=$ywv
...
proc doChangeType(pkty)
	variable  pkty
	silent 1 ; pauseupdate
	variable peak=root:PF:peak
	root:PF:fittype[peak]=pkty
	if(pkty==1) //lor
		root:PF:fitparms[3][peak]=nan								//GW
		root:PF:fitparms[4][peak]=nan								//ASYM
	endif
	if(pkty==2) //lor-gauss
		if(numtype(root:PF:fitparms[2][peak])==2)
			root:PF:fitparms[2][peak]=LWguess						//LW
		endif
		if(numtype(root:PF:fitparms[3][peak])==2)
			root:PF:fitparms[3][peak]=GWGuess					//GW
		endif
		root:PF:fitparms[4][peak]=nan								//ASYM
	endif
	if(pkty==3) //lor-don
		...
		root:PF:fitparms[3][peak]=nan								//GW
		if(numtype(root:PF:fitparms[4][peak])==2)
			root:PF:fitparms[4][peak]=ASYMguess						//LW
		endif
	endif
	...
	if(pkty==5) //gauss
		root:PF:fitparms[2][peak]=nan								//LW
		if(numtype(root:PF:fitparms[3][peak])==2)
			root:PF:fitparms[3][peak]=GWGuess					//GW
		endif
		root:PF:fitparms[4][peak]=nan								//ASYM
...
proc DoGuessPrefs(lg,gg,ag,sg,rg)
	variable lg=root:PF:LWGuess,gg=root:PF:GWGuess, ag=root:PF:ASYMguess,sg=root:PF:SPLITguess,rg=root:PF:RATIOguess
	...
	root:PF:LWGuess=lg
	root:PF:GWGuess=gg
	root:PF:ASYMguess=ag
	root:PF:SPLITguess=sg
	root:PF:RATIOguess=rg
end
...
proc doFitChannel(quiet)
	...
	variable tstart
	tstart=datetime
	makeConstraints() //; print "makeC:",datetime-tstart
	variable hasConstr=0
	if(numpnts(root:PF:constraints)>0)
		hasConstr=1
	endif
	if (hasConstr)
		cmd+="/c"
	...
	if (root:PF:isSeries)
		cmd+=" FitFunction :PF:fitparms root:PF:singlechannel"
	else
		cmd+=" FitFunction :PF:fitparms :"+root:PF:ywv  
	endif	
	if(strlen(root:PF:xwv))
		cmd+=" /x=" +root:PF:xwv
	endif
	cmd+=" /D=:PF:peakfit/R"
	if(hasConstr)
   ...
   //print "before cmd:",datetime-tstart
...
//*-*
	//tstart=datetime
	root:PF:fitparms[2,3][1,]=abs(fitparms[p][q])
	if(!quiet)
		updatePFPanel()
	endif
	print "Fittime(proc)=",datetime-tstart," sec"
end
...
//be above root:PF
proc doFitChannels(cstart, cend, checkNAN)
	variable cstart=root:PF:channel, cend=root:PF:nchan-1, checknan=2
	prompt cstart, "Enter starting channel"
	prompt cend, "Enter ending channel"
	prompt checknan, "Check for NaN waves", popup "no;yes"
	variable c=cstart, step=1, condn
	variable skip, t0, t1, t2, t3, t4, t5
	if(cend<cstart)
		step=-1
	endif
	do
		...
		//print "Channel=",c
		:PF:channel=c
		copychannel(c)//; t1=datetime-t0; print "copy:",t1
		if (checknan-1)
			wavestats :PF:Singlechannel
			skip=(0==v_npnts)
		else
			skip=0
		endif
		if(!skip)
			doFitChannel(1)//; t2=datetime-t1-t0; print "dofit:",t2
			doFit2History(1)//; t3=datetime-t2-t1-t0; print "c2hist:",t3 
			calcHistory(c)//; t4=datetime-t3-t2-t1-t0; print "calchist:", t4
		endif
		c+=step
		if(cend<cstart)
			condn=(c>=cend)
...
proc makeConstraints()
	silent 1; pauseupdate
	make/t/o/n=(14*root:PF:numpks) root:PF:constraints
	variable pk=1, pk1, k0, k1, v,j,nc=0,isNan
	string s, op, cmd
	do
		j=0
		do
			isNan=numtype(root:PF:fitparms[j][pk])==2
			if((root:PF:hold[j][pk][0]==3)*(!isNan))	//relative to
				k0=j+(pk)*7
				pk1=root:PF:hold[j][pk][2]
				k1=j + (pk1) * 7
				v=root:PF:hold[j][pk][1]
				...
				root:PF:constraints[nc]=s
				s="k"+num2str(k0) + ">" + num2str(v) + op + "k"+num2str(k1)
				root:PF:constraints[nc+1]=s
				if ((j==0)+(j==5))		//force constraint on parms
					root:PF:fitparms[j][pk]=fitparms[j][pk1] +v//+ v
				else
					root:PF:fitparms[j][pk]=fitparms[j][pk1] * v
				endif
				nc+=2
			endif
			if((root:PF:hold[j][pk][0]==2)*(!isNan))	//between
				if (numtype(root:PF:minmax[j][pk][0])==0) 	//low value
					k0=j+(pk)*7
					v=root:PF:minmax[j][pk][0]
					s="k"+num2str(k0)+">" +num2str(v)
					root:PF:constraints[nc]=s
					if(root:PF:fitparms[j][pk]<v)	//force constraint on parms
						root:PF:fitparms[j][pk]=v
					endif
					nc+=1
				endif
				if (numtype(root:PF:minmax[j][pk][1])==0) 	high value
					k0=j+(pk)*7
					v=root:PF:minmax[j][pk][1]
					s="k"+num2str(k0)+"<" +num2str(v)
					root:PF:constraints[nc]=s
					if(root:PF:fitparms[j][pk]>v)	//force constraint on parms
						root:PF:fitparms[j][pk]=v
		...
		while(j<7)
		pk+=1
	while(pk<=root:PF:numpks)
	redimension/n=(nc) root:PF:constraints
end
...
Window pfpanel() : Panel
	...
	ValDisplay valdisp0,limits={0,0,0},barmisc={0,1000},value= #"root:PF:numpks"
	...
	CheckBox check0,pos={380,116},size={51,14},title="history",proc=histCheckProc
...
proc CopyParms()
	//hold popup menus
	variable/g v_value
	controlinfo/w=PFPanel poshold ;	root:PF:hold[0][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel amphold ;	root:PF:hold[1][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel lwhold ;	root:PF:hold[2][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel gwhold ;	root:PF:hold[3][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel asymhold ;	root:PF:hold[4][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel splithold ;	root:PF:hold[5][root:PF:peak][0]=::v_value-1
	controlinfo/w=PFPanel ratiohold ;	root:PF:hold[6][root:PF:peak][0]=::v_value-1

	//numerical controls
	root:PF:hold[0][root:PF:peak][2]=posrelpeak		;root:PF:hold[0][root:PF:peak][1]=posrelval
	root:PF:hold[1][root:PF:peak][2]=amprelpeak		;root:PF:hold[1][root:PF:peak][1]=amprelval
	root:PF:hold[2][root:PF:peak][2]=lwrelpeak		;root:PF:hold[2][root:PF:peak][1]=lwrelval
	root:PF:hold[3][root:PF:peak][2]=gwrelpeak		;root:PF:hold[3][root:PF:peak][1]=gwrelval
	root:PF:hold[4][root:PF:peak][2]=asymrelpeak		;root:PF:hold[4][root:PF:peak][1]=asymrelval
	root:PF:hold[5][root:PF:peak][2]=splitrelpeak		;root:PF:hold[5][root:PF:peak][1]=splitrelval
	root:PF:hold[6][root:PF:peak][2]=ratiorelpeak		;root:PF:hold[6][root:PF:peak][1]=ratiorelval
	
	root:PF:minmax[0][root:PF:peak][0]=poslo			;root:PF:minmax[0][root:PF:peak][1]=poshi
	root:PF:minmax[1][root:PF:peak][0]=amplo		;root:PF:minmax[1][root:PF:peak][1]=amphi
	root:PF:minmax[2][root:PF:peak][0]=lwlo			;root:PF:minmax[2][root:PF:peak][1]=lwhi
	root:PF:minmax[3][root:PF:peak][0]=gwlo			;root:PF:minmax[3][root:PF:peak][1]=gwhi
	root:PF:minmax[4][root:PF:peak][0]=asymlo		;root:PF:minmax[4][root:PF:peak][1]=asymhi
	root:PF:minmax[5][root:PF:peak][0]=splitlo		;root:PF:minmax[5][root:PF:peak][1]=splithi
	root:PF:minmax[6][root:PF:peak][0]=ratiolo		;root:PF:minmax[6][root:PF:peak][1]=ratiohi
end	
...
proc SetFixed()
	variable/g v_value
	controlinfo/w=PFPanel fix0;	root:PF:hold[0][0][0]=::v_value
	controlinfo/w=PFPanel fix1;	root:PF:hold[1][0][0]=::v_value
	controlinfo/w=PFPanel fix2;	root:PF:hold[2][0][0]=::v_value
	controlinfo/w=PFPanel fix3;	root:PF:hold[3][0][0]=::v_value
	controlinfo/w=PFPanel fix4;	root:PF:hold[4][0][0]=::v_value
	//controlinfo/w=PFPanel fix5;	root:PF:hold[5][0][0]=::v_value
	//controlinfo/w=PFPanel fix6;	root:PF:hold[6][0][0]=::v_value
	controlinfo/w=PFPanel fixFEPos;	root:PF:hold[5][0][0]=::v_value
	controlinfo/w=PFPanel fixFEWid;	root:PF:hold[6][0][0]=::v_value
end
...
proc copyBkgdParms()
	variable/g v_value
	controlinfo/w=PFPanel bkgd0; root:PF:fitparms[0][0]=::v_value
	controlinfo/w=PFPanel bkgd1; root:PF:fitparms[1][0]=::v_value
	controlinfo/w=PFPanel bkgd2; root:PF:fitparms[2][0]=::v_value
	controlinfo/w=PFPanel bkgd3; root:PF:fitparms[3][0]=::v_value
	controlinfo/w=PFPanel bkgd4; root:PF:fitparms[4][0]=::v_value
//	controlinfo/w=PFPanel bkgd5; root:PF:fitparms[5][0]=::v_value
//	controlinfo/w=PFPanel bkgd6; root:PF:fitparms[6][0]=::v_value
	controlinfo/w=PFPanel FEPos; root:PF:fitparms[5][0]=::v_value
	controlinfo/w=PFPanel FEWidth; root:PF:fitparms[6][0]=::v_value
end
		
Function fit2history(ctrlName) : ButtonControl
	String ctrlName
	execute "doFit2History(0)"
End

//not in PF
//if mode=0 then  calchistory for all channnels at end 
proc doFit2History(mode)
	variable mode=0
	root:PF:hist_fitparms[][0,root:PF:numpks][root:PF:channel]=fitparms[p][q][channel]
	root:PF:hist_fittype[0,root:PF:numpks][root:PF:channel]=fittype[p][channel]
	root:PF:hist_npks[root:PF:channel]=numpks
	root:PF:hist_chisq[root:PF:channel]=::v_chisq
	root:PF:hist_isStored[root:PF:channel]=1
	root:PF:hist_hold[][][][root:PF:channel]=hold[p][q][r]
	if (!mode)
		calchistory(-1)
	endif
...
//not in PF
proc doKillHistory()
	root:PF:hist_fitparms[][0,][root:PF:channel]=nan
	root:PF:hist_fittype[0,][root:PF:channel]=nan
	root:PF:hist_npks[root:PF:channel]=nan
	root:PF:hist_chisq[root:PF:channel]=nan
	root:PF:hist_isStored[root:PF:channel]=nan
	root:PF:hist_hold[][][][root:PF:channel]=nan
	calchistory(-1)
end
...
//not in PF
proc doHistory2Fit()
	if(root:PF:hist_isStored[root:PF:channel])
		root:PF:numpks=hist_npks[channel]
		redimension/n=(7,root:PF:numpks+1) root:PF:fitparms
		root:PF:fitparms[][]=hist_fitparms[p][q][channel]
		redimension/n=(root:PF:numpks+1) root:PF:fittype
		root:PF:fittype[][]=hist_fittype[p][channel]
		redimension/n=(7,root:PF:numpks+1,3) root:PF:hold
		root:PF:hold[][][]=hist_hold[p][q][r][channel]
		updatePFPanel()
	endif
end
...
proc doAnimate(ch0,ch1)
	...
	iterate((ch1-ch0)+1)
		...
		if(i==0)
			...
			wavestats/q root:PF:hist_npks
			variable pkmax=v_max
			iterate(pkmax)
				wvn_="root:PF:hist_peak"+num2str(i+1)
				np=numpnts(root:PF:singlechannel)+1
				make/o/n=(np,(ch1-ch0)+1) $wvn_
			loop
			duplicate/o $wvn_ root:PF:hist_peakx root:PF:hist_peakfit root:PF:hist_bkgd root:PF:hist_dataFit
			if(strlen(root:PF:xwv)>0)
				hist_peakx=root:PF:singlechannel_x[p]
			else
				root:PF:hist_peakx=p*dimdelta(singlechannel,0)+dimoffset(singlechannel,0)
			endif

		endif
		root:PF:hist_peakfit[0,np-2][i]= peakfit[p]; root:PF:hist_peakfit[np-1][i]= nan
		root:PF:hist_bkgd[0,np-2][i]= bkgdfit[p]; root:PF:hist_bkgd[np-1][i]= nan
		root:PF:hist_dataFit[0,np-2][i]= singlechannel[p]; root:PF:hist_dataFit[np-1][i]= nan
		iterate(pkmax)
			wvn="peak"+num2str(i+1)
			wvn_="root:PF:hist_peak"+num2str(i+1)
			if(i<root:PF:hist_npks[j])
				$wvn_[0,np-2][j]=$wvn[p]
			else
				$wvn_[0,np-2][j]=nan
			endif
...
proc makeFitStack()
	duplicate/o root:PF:hist_peakfit root:PF:hist_peakFitStack
	duplicate/o root:PF:hist_datafit root:PF:hist_dataFitStack
	duplicate/o root:PF:hist_peakx root:PF:hist_peakxStack
	root:PF:hist_peakFitStack:=hist_peakfit[p][q]+q*offsetStackPF
	root:PF:hist_dataFitStack:=hist_datafit[p][q]+q*offsetStackPF	
	root:PF:hist_peakxStack:=hist_peakX[p][q]+q*shiftStackPF
	dowindow/f FitStack
	if(v_flag==0)
		FitStack()
	endif
...
//if chan is -1 then refresh calc for all xhannels
macro calcHistory(chan)
	variable chan=-1
	silent 1; pauseupdate
	string npk="root:PF:numpks"
	wavestats/q root:PF:hist_npks
	variable npkmax=v_max
	variable nch=root:PF:nchan

	if (chan==-1)
		root:PF:hist_POS=nan
		root:PF:hist_AMP=nan
		root:PF:hist_LW=nan
		root:PF:hist_GW=nan
		root:PF:hist_ASYM=nan
		root:PF:hist_SPLIT=nan
		root:PF:hist_RATIO=nan
		root:PF:hist_POS[0,nch-1][1,npkmax] = hist_fitparms[0][q][p]
		root:PF:hist_AMP[0,nch-1][1,npkmax] = hist_fitparms[1][q][p]
		root:PF:hist_LW[0,nch-1][1,npkmax] = hist_fitparms[2][q][p]
		root:PF:hist_GW[0,nch-1][1,npkmax] = hist_fitparms[3][q][p]
		root:PF:hist_ASYM[0,nch-1][1,npkmax] = hist_fitparms[4][q][p]
		root:PF:hist_SPLIT[0,nch-1][1,npkmax] = hist_fitparms[5][q][p]
		root:PF:hist_RATIO[0,nch-1][1,npkmax] = hist_fitparms[6][q][p]
	else
		root:PF:hist_POS[chan][1,npkmax] = hist_fitparms[0][q][p]
		root:PF:hist_AMP[chan][1,npkmax] = hist_fitparms[1][q][p]
		root:PF:hist_LW[chan][1,npkmax] = hist_fitparms[2][q][p]
		root:PF:hist_GW[chan][1,npkmax] = hist_fitparms[3][q][p]
		root:PF:hist_ASYM[chan][1,npkmax] = hist_fitparms[4][q][p]
		root:PF:hist_SPLIT[chan][1,npkmax] = hist_fitparms[5][q][p]
		root:PF:hist_RATIO[chan][1,npkmax] = hist_fitparms[6][q][p]
	endif
end
...
Function HistCheckProc(cba) : CheckBoxControl
...
proc UpdateBrowser()
	...
	if(root:PF:whichHistBrowse==0)
	print s_value
		duplicate/o $"root:PF:"+s_value root:PF:browse_x 
		root:PF:browse_y:=$::s_value
	else
		root:PF:browse_y=$::s_value[p][whichHistBrowse]
	endif
	label left s_value
	controlinfo/w=HistoryBrowser bottomAxis
	if(root:PF:whichHistBrowse==0)
		duplicate/o $"root:PF:"+s_value root:PF:browse_x 
		root:PF:browse_x:=$::s_value
	else
		root:PF:browse_x=$::s_value[p][whichHistBrowse]
	endif
...
proc doTranspose()
	controlinfo/w=HistoryBrowser leftAxis
	variable/g root:PF:tempv=root:v_value
	controlinfo/w=HistoryBrowser bottomAxis
	popupmenu leftaxis win=HistoryBrowser,mode=v_value
	popupmenu bottomaxis win=HistoryBrowser,mode=root:PF:tempv
	updateBrowser()
end
...
proc doExportHistory(pref,peaknum, disp)
	string  pref=root:PF:lastExportName
	variable peaknum=root:PF:whichHistBrowse, disp
	...
	do
		...
		np=dimsize($"root:PF:"+s_value,0)
		make/o/n=(np) $leftname
		$leftname=$"root:PF:"+s_value[p][i]
		controlinfo/w=HistoryBrowser bottomAxis
		bottomname=pref+"_"+num2str(i)+"_"+s_value[5,100]
		make/o/n=(np) $bottomname
		$bottomname=$"root:PF:"+s_value[p][i]	
