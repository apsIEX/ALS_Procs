...
function extractLineProc(ctrlName):ButtonControl
	string ctrlname
	string df=getdf()
	variable xx=0,yy=0,np=200
	string phirange="0",wvout="wvout"
	...
	doprompt "enter lineout information",wvout,xx,yy,np,phirange
	...
	variable xmax=max(x0,x1), xmin=min(x0,x1), ymax=max(y0,y1),ymin=min(y0,y1)
	//check point is inside the window
	if((xx>=xmin)*(xx<=xmax)*(yy>=ymin)*(yy<=ymax))
	else
		abort "must have point inside the data range"
	endif
	//parse phirange variables
	variable phistart, phiend, phiN
	phistart=str2num(stringfromlist(0,phirange,","))
	if (itemsinlist(phirange,",")==3)
		phiend=str2num(stringfromlist(1,phirange,","))
	...
	setscale x xmin, xmax, ilt
	setscale y ymin,ymax,ilt

	for(phi=phistart;phi<=phiend;phi+=(phiend-phistart)/(phiN-1))
		ilt=abs(atan2(yy-y,xx-x))==abs(phi)*pi/180
	endfor
...
function setupcontrols(df)
	...
	popupmenu Process,mode=0,pos={290,22},size={100,20},proc=IT5process,title="Process",value="Zap Nans"
end
