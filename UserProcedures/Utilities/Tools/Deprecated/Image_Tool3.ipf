//File: Image_Tool3			Created: 6/98
// Jonathan Denlinger, JDDenlinger@lbl.gov
// 10/98 jdd major additions
// 10/20/98 jdd added Modify procedures
// 11/1/98 jdd added Analyze menu
// 12/12/98 jdd added Marquee items & balloon help
// 1/26/99 jdd rearranged control bar buttons; smaller title font
// 8/11/99 jdd (1) added 'Find Peak Max' option to Analyze; 
//                        (2) Added custom convolve filters AvgX, AvgY & # of passes selection
//                        (3) combined Undo & Restore buttons to a single Popup menu
//                        (4) added stack increment selection
//                        (5) added cursor hair step buttons to upper right blank area
//                        (6) add version number 'v2.0'  to window to help keep track of future changes
// 12/4/99 er added live update of crosshair using window hook function (CMD+mouse)
// 1/17/00 jdd  added 3D data set slicing (v3.0)
// 2/11/00 jdd added Gamma and Gray/RedTemp selection
// 2/20/00 jdd  moved 3D tools to ZProfile window; improve Animation options
// 6/6/00  er added live update of zprofile slice selection. CMD + mouse selects the slice
// 6/10/00 jdd  turned on auto-rescale ColorTable for every slice selection
// 9/23/00 jdd (v3.0) remove step/center buttons & put functionality into hook function with OPT/SHIFT+mouse
//                        added "?" to popup Help Notebook containing shortcut information
// 4/23/01 jdd made "Norm X" behave identical to "Norm Y"
//  5/26/01 jdd  (v3.5) added Invert to SelectCT options; added working Shift processing option
//  11/11/01 jdd  (v3.6) added "Full X" option to ConfirmXnorm (& Y)
//  2/19/02 jdd (3.7) added "EVENT:mouse" checking in hook functions for v4.05A of Igor
//  2/24/02 jdd   added  cross hairs to line profile plots & modified live updating behavior
//  4/7/02  jdd  (3.8) revamped "ResizeImg" parameters and added interp2D capability
//  6/3/02  jdd  (3.9) add OS specific load panel resize
//  6/26/02 jdd   fixed update of crosshairs when doing transpose, undo/restore
//  7/8/02  jdd  () repackaged Crop; added use of Crop() marquee outside of ImageTool
...
Menu "2D"
	"-"
	"Image Tool 3!"+num2char(19), ShowImageTool()
...
Proc ImageToolHelp()
	...
	if (V_flag==0)
		string txt
		NewNotebook/W=(100,100,470,400)/F=1/N=ImageToolInfo
		Notebook ImageToolinfo, showruler=0, backRGB=(45000,65535,65535)
		Notebook ImageToolinfo, fstyle=1, text="Image Tool 3\r"
		Notebook ImageToolinfo, fstyle=0, text="version 3.91, 6/27/02 J. Denlinger\r\r"
		
		Notebook ImageToolinfo, fstyle=1, text="Mouse Shortcuts:\r"
		Notebook ImageToolinfo, fstyle=2, text="Image Tool window\r"
		...
		Notebook ImageToolinfo, fstyle=2, text="Z-profile window\r"
...
//---------
	...
	NewDataFolder/O/S root:IMG
		...
		Make/O HairZ0={-Inf,0, Inf}
		SetScale/P x 0,1E-7,"" HairZ0
		variable/G ndim, islice, nz, iz, Z0, islicedir=1
		...
		image=cos((pi/10)*sqrt(x^2+y^2))*cos( (2.5*pi)*atan2( y, x))
...
//  Modifier bits:  0001=mousedown, 0010=shift  , 0100=option/alt, 1000=cmd/ctrl
	string s
	variable mousex,mousey,ax,ay, modif, returnval=0
...
//--------------------
	...
	//print "AdjustCT: ", V_min, V_max	//, root:V_min, root:V_max
	//variable/G root:IMG:dmin=root:V_min, root:IMG:dmax=root:V_max
...
//======================
	...
	prompt eopt, "Export", popup, "Image & CT;Color Table only;X profile;Y profile;Z profile"
...
Window ImageTool() : Graph
	...
	TextBox/N=text0/F=0/X=104.13/Y=-15 "\\Z10v3.91"
	ControlBar 47
...
Proc Zaction(ctrlName,popNum,popStr) : PopupMenuControl
//---------------------------
	String ctrlName
	Variable popNum
	String popStr
