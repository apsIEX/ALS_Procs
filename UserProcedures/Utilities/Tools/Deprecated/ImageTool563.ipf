...
function tabproc(name,tab)	
   ...
   // Lineout ==============================
	...
	checkbox cNorm1_CHeckBox,disable=(tab!=7)
	...
	checkbox cNorm2_CHeckBox,disable=(tab!=7)
...
//	checkbox invertCT,pos={153,45},size={80,14},title="Invert?"
	...
	CheckBox cNorm1_CheckBox pos={cstarth,vstarth+3},title=""

	popupmenu cNorm1_Menu,mode=1,pos={cstarth+15,vstarth},size={43,20},proc=NormMenu,title="",value="off;1D;2D;3D"
	...
	CheckBox cNorm2_CheckBox pos={cstarth,vstarth+3},title=""

	popupmenu cNorm2_Menu,mode=1,pos={cstarth+15,vstarth},size={43,20},proc=NormMenu,title="",value="off;1D;2D;3D"
...
function setupV(dfn,w)
	...
	variable /g $(dF+"gamma")
	nvar gamma=$(dF+"gamma")
//	setformula $(df+"pmap") , "255*(p/255)^(abs("+df+"gamma))"
...
function linkfunc_cursor(df0,df1,linkoffset,x0,x1,y0,y1,z0,z1,t0,t1)
	...
	for(i=0;i<4;i+=1)
		...
		IF (linkaxis==1)
			NVAR c0 = $(axisvariable(df0,dnum0[i]))
			NVAR c1 = $(axisvariable(df1,dnum1[i]))
...
function doMarqueeExpand(df , wn,menuitem)
