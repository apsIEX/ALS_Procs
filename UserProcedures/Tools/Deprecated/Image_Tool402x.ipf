...
//  2/21/03 JD  - (4.02) Added Export TabControl and rewrote export subroutines as separate button control functions
...
//---------
	...
	NewDataFolder/O/S root:ImageTool
	...
	NewDataFolder/O/S root:ImageTool:STACK
...
Window ImageTool() : Graph
	...
	PopupMenu popAnim,mode=0,value= #anim_menu
	...
	ValDisplay version,pos={600,1},size={30,14},fsize=9, title="v4.02",frame=0
...
Proc PickImage( wn )
//------------
	String wn=StrVarOrDefault(getdf()+"imgnam","")
...
//============
	...
	string WaveNamList=TraceNameList("ImageTool", ";",1)+ImageNameList("ImageTool", ";")
...
//--------------------
	...
	if (idir==1)	// XZ
		//profileZ:=dw(X0)(x)(Y0)
		execute df+"profileZ:=root:"+datnam+"("+df+"X0)(x)("+df+"Y0)"   
	endif
	if (idir==2)	// XY
		//profileZ:=dw(X0)(Y0)(x)
		execute df+"profileZ:=root:"+datnam+"("+df+"X0)("+df+"Y0)(x)"   
	endif
...
//----------
	...
	IF ((popNum>=3)*(popNum<=11))
		...
		if ((popNum>=10)*(popNum<=11))
			...
			$(df+"anim_range")=popNum-10
		endif
		$(df+"anim_menu")=newmenu
		PopupMenu popAnim value=$(df+"anim_menu")
	ELSE
		
		variable istart, iend, istep, idir=$(df+"anim_dir")
		istart=SelectNumber( $(df+"anim_range"), 0, pcsr(A) )
		iend=SelectNumber( $(df+"anim_range"), $(df+"nz")-1, pcsr(B) )
		istep=$(df+"zstep") * sign( iend-istart)
...
//================  for Image_Tool
	...
	case "Shift":
		...
		opt=SelectString(shiftdir==1, "/X","/Y" ) +"/O=Image/E="+num2str(shift_expand)
		cmd="VolShift( root:"+datnam+", root:"+shiftwn+",\"" +opt+"\")"
		print cmd
...
//------------
	String axis=StrVarOrDefault(getdf()+"axis_norm", "X" )
	Variable/C xr=Cmplx( NumVarOrDefault(getdf()+"x1_norm", 0 ), NumVarOrDefault(getdf()+"x2_norm", 1 ) )
	Variable/C yr=Cmplx( NumVarOrDefault(getdf()+"y1_norm", 0 ), NumVarOrDefault(getdf()+"y2_norm", 1 ) )
...
//------------
	String axis=StrVarOrDefault(getdf()+"axis_norm", "X" )
	Variable x1=NumVarOrDefault(getdf()+"x1_norm", 0 )
	Variable x2=NumVarOrDefault(getdf()+"x2_norm", 1 )
	Variable y1=NumVarOrDefault(getdf()+"y1_norm", 0 )
	Variable y2=NumVarOrDefault(getdf()+"y2_norm", 1 )
	Prompt axis, "Normalization Axis", popup, "X;Y;XY;YX"
...
//------------
	String shftwn=StrVarOrDefault(getdf()+"shiftwn", "" )
	Variable dir=NumVarOrDefault(getdf()+"shiftdir", 1 )+1
	Variable expand=NumVarOrDefault(getdf()+"shift_expand", 1 )+2
	prompt shftwn, "Shift Wave Name", popup, WaveList("!*_x",";","DIMS:1")
	prompt dir, "Shift Direction", popup, "X;Y"
...
//------------
	string xrang=num2str($(getdf()+"xmin"))+", "+num2str($(getdf()+"xmax"))+", "+num2str($(getdf()+"xinc"))
	string yrang=num2str($(getdf()+"ymin"))+", "+num2str($(getdf()+"ymax"))+", "+num2str($(getdf()+"yinc"))
	variable xopt, yopt
...
//------------
	variable opt
	string xrang=num2str($(getdf()+"x1_resize"))+", "+num2str($(getdf()+"x2_resize"))
	string yrang=num2str($(getdf()+"y1_resize"))+", "+num2str($(getdf()+"y2_resize"))
...
// investigate ImageInterpolate (Igor 4) as option for making smaller 
	variable xyopt
	string xyval=StrVarOrDefault(getdf()+"N_resize", "1,1" )
...
//============
	...
	//setdataFolder $df
		...
		//WAVE Gray_CT=Gray_CT, RedTemp_CT=RedTemp_CT
	//setdatafolder $curr
	string CTstr
	PauseUpdate
	switch( popNum )
	case 1:
		...
		//Image_CT:=Gray_CT[pmap[p]][q]		//can't use local variable in dependency 
		//himg_ct:=Gray_CT[pmap[p]][q]
		//vimg_ct:=Gray_CT[pmap[p]][q]
			//$(df+"Image_CT"):=$(df+"Gray_CT")[pmap[p]][q]
			//$(df+"himg_ct"):=$(df+"Gray_CT")[pmap[p]][q]
			//$(df+"vimg_ct"):=$(df+"Gray_CT")[pmap[p]][q]
		break
	...
	case 2:
		...
		execute df+"Himg_CT:="+df+CTstr
		execute df+"Vimg_CT:="+df+CTstr
		//$(df+"Image_CT"):=$(df+"RedTemp_CT")[pmap[p]][q]
		//$(df+"himg_ct"):=$(df+"RedTemp_CT")[pmap[p]][q]
		//$(df+"vimg_ct"):=$(df+"RedTemp_CT")[pmap[p]][q]
		break
...
//---------------
	...
	if (cmpstr(ctrlName,"setShift")==0)
		$(df+"STACK:shift")=varNum
	else
		$(df+"STACK:offset")=varNum
	endif
	
	OffsetStack( $(df+"STACK:shift"), $(df+"STACK:offset"))
End
...
//------------
	...
	string wvn=CsrWave(A)
		ycur=$(df+"STACK:ymin") + $(df+"STACK:yinc") * str2num( wvn[4,strlen(wvn)-1] )
		DoWindow/F ImageTool
		ModifyGraph offset(HairY0)={xcur, ycur}
		Cursor/P A, profileH, round((xcur - DimOffset($(df+"Image"), 0))/DimDelta($(df+"Image"),0))
		Cursor/P B, profileV_y, round((ycur - DimOffset($(df+"Image"), 1))/DimDelta($(df+"Image"),1))
	endif
End
...
Window Stack_() : Graph
	PauseUpdate; Silent 1		// building window...
	String df=getdf(), fldrSav= GetDataFolder(1)
