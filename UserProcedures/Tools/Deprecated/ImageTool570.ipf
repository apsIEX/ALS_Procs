...
#include "colorTables"
	static constant MAJOR_VERSION = 5.70
	...
	//5.65 add a norm2D 
	//5.66 Add higher resolution colour tables
	//		 add normalize image histogram function
	//5.67 make it possible to update older imagetools
	//		add hook function to experiment load that automatically updates all old tools
	//5.68 AB ?
	//5.69 ER added gaussian braoden to process tab
	//5.70 ER added axis size sliders
	//			 added cursor hide option
	//			 reduced gap between axes from .05 to .02
...
function SetupVariables(dfn)
	...
	makevariable(df,"setXavg",1)
	makevariable(df,"setYavg",1)
	makevariable(df,"setZavg",1)
	makevariable(df,"setTavg",1)
	makevariable(df,"AvgMMode",1)
...
function tabproc(name,tab)	
	...
	checkbox HistNormCT,disable=(tab!=2)
	...
	checkbox HideCursors,disable=(tab!=3)
	slider sliderLeft,disable=(tab!=3)
	slider sliderBottom,disable=(tab!=3)
	// Export ==============================
   ...
   // Lineout ==============================
	...
	SetVariable setbin0,disable=(tab!=6),title=axistitle(df,0)
	SetVariable setbin1,disable=(tab!=6),title=axistitle(df,1)
	SetVariable setbin2,disable=(tab!=6),title=axistitle(df,2)
	SetVariable setbin3,disable=(tab!=6),title=axistitle(df,3)
	...
	SetVariable setLinkOffset0, disable=(tab!=8),title=axistitle(df,0),limits={-inf,inf,dimdelta(w,0)}
	SetVariable setLinkOffset1, disable=(tab!=8),title=axistitle(df,1),limits={-inf,inf,dimdelta(w,1)}
	SetVariable setLinkOffset2, disable=(tab!=8),title=axistitle(df,2),limits={-inf,inf,dimdelta(w,2)}
	SetVariable setLinkOffset3, disable=(tab!=8),title=axistitle(df,3),limits={-inf,inf,dimdelta(w,3)}
...
function setupcontrols(df)
	...
	PopupMenu imgHOpt,mode=1,proc=ROIColorOption,value=#imgopstring
	PopupMenu imgOpt,pos={x0+9,y0+36},size={80,14},title="main "
	PopupMenu imgOpt,mode=1,proc=ROIColorOption,value=#imgopstring
...
Slider sliderBottom,pos={cstarth,vstarth+47},size={53,13},proc=SliderAxisProc
	...
	SetVariable setLinkOffset0,pos={cstarth,vstarth},size={70,14},title=axistitle(df,0)
	...
	SetVariable setLinkOffset1,pos={cstarth,vstarth+vstep},size={70,14},title=axistitle(df,1)
	...
	SetVariable setLinkOffset2,pos={cstarth+cstep,vstarth},size={70,14},title=axistitle(df,2)
	execute "SetVariable setLinkOffset2,limits={-inf,inf,"+num2str(dimdelta(w,2))+"},value="+df+"LinkOffset[2], bodywidth=40"
	CheckBox setLink2 pos={cstarth+cstep +bstep,vstarth},title=""
	execute "checkbox setLink2,variable="+df+"LinkAxis2"

	SetVariable setLinkOffset3,pos={cstarth+cstep,vstarth+vstep},size={70,14},title=axistitle(df,3)
	...
	strswitch(popstr)
		...
		case "Convolve Gaussian":
			...
			if(!v_flag)
				...
				ifft convTemp
				copyscales img,convTemp
				duplicate/o convTemp root:convOut
				wave co=root:convOut
				display; appendimage co
...
function setupV(dfn,w)
	...
	if(!ZTPmode)
		if(hasZP)
			...
			modifygraph axisEnab(profzb)={bottomstop+AxisGap,1},axisenab(profZR)={leftstop+axisGap+0.5*(1-leftstop)*hasTP,1},freepos(profZR)={inf,profZB}
		...
		if(hasTP)
			...
			modifygraph axisEnab(profTb)={bottomstop+axisGap,1},axisenab(profTR)={leftstop+axisGap,1-0.5*(1-leftstop)*hasZP},freepos(profTR)=0
