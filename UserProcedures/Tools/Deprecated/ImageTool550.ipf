...
static constant MAJOR_VERSION = 5.2
...
function/s wv4di(wv,df,wvout,sarr,iarr)
	...
	if(dimsize(wout,0)!=0)
	if (j==4) 
		string nn="Norm1"//+num2str(Norm_num)
		NVAR Mode= $(df+nn+"_Mode")
		//	sout= df+wvout +"=0;"+df+wvout+"_trig:=Extractplane("+df+wvout+","+ wv+","+df+wvout+"_axis,"+swv2[2]+","+swv2[3]+")"
		if(mode==2)
				sout= df+wvout +"=0;"+df+wvout+"_trig:=Extractplanebinnorm("+df+wvout+","+ wv+","+df+wvout+"_axis,"+df+"bin,"+swv2[2]+","+swv2[3]+","+df+"norm1_Scale)"
		else
...
function setupcontrols(df)
	...
	PopupMenu imgHOpt,mode=1,value=#imgopstring,proc=ROIColorOption
	PopupMenu imgOpt,pos={x0+9,y0+36},size={80,14},title="main "
	PopupMenu imgOpt,mode=1,value=#imgopstring,proc=ROIColorOption
	PopupMenu imgZTOpt,pos={x0+100,y0+15},size={80,14},title="corner"
	PopupMenu imgZTOpt,mode=1,value=#imgopstring,proc=ROIColorOption
	PopupMenu imgVOpt,pos={x0+100,y0+36},size={80,14},title="vert   "
	PopupMenu imgVOpt,mode=1,value=#imgopstring,proc=ROIColorOption
...
Function NormMenu(PU_Struct)
	...
	switch(PU_Struct.popnum)	
		case 1:
			Mode=1
			string Norm_name=df+"Norm"+num2str(Norm_Num)+"_Scale"
			 execute Norm_name+"_trig=0"
			setupV(getdfname(),dname)
			break
		case 2:		
			Mode=2
			NormSetup(df,dname,nn,Norm_Num,axis,2)
			setupV(getdfname(),dname)
			break
		case 3:		
			break
...
Function NormSetup(df,dname,nn,Norm_Num,axis,method)
	...
	dims[axis]=1
	string ANorm_name=df+"Norm"+num2str(Norm_Num)+"_Scale"
	if(method==0)
	...
	trig=0
	if(method==0)
		execute ANorm_name+"_trig:=calcnorm("+ANorm_name+","+dname+","+num2str(axis)+","+df+nn+"_p0,"+df+nn+"_p1)"
	else
		execute ANorm_name+"_trig:=calcnorm2("+ANorm_name+","+dname+","+num2str(method)+","+num2str(axis)+","+df+nn+"_p0,"+df+nn+"_p1)"
	endif
end
